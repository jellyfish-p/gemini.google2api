<template>
  <main class="min-h-screen bg-linear-to-br from-slate-50 to-slate-100/60 text-slate-900">
    <!-- Login Screen -->
    <section v-if="!authenticated" class="relative flex min-h-screen items-center justify-center px-4 py-10">
      ...
      <UCard class="relative w-full max-w-md shadow-xl shadow-slate-200/60 ring-1 ring-slate-200/50">
        <template #header>
          <div class="space-y-1">
            <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-linear-to-br from-primary-500 to-primary-600 shadow-sm ring-1 ring-white/20">
    ...
    <!-- Admin Dashboard -->
    <section v-else class="flex h-screen flex-col overflow-hidden">
      <!-- Top Header (full width) -->
      <header class="shrink-0 border-b border-slate-200/80 bg-white/90 px-4 py-3 backdrop-blur-xl lg:px-6">
        <div class="flex items-center justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2.5">
              <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-linear-to-br from-primary-500 to-primary-600 text-white shadow-sm">
                <UIcon name="i-lucide-gem" class="size-5" />
              </div>
              <div class="hidden sm:block">
                <div class="text-sm font-semibold text-slate-900">{{ t('admin.brand') }}</div>
                <div class="text-xs text-slate-500">{{ t('admin.brandSubtitle') }}</div>
              </div>
            </div>
            <div class="hidden h-6 w-px bg-slate-200 md:block" />
            ...
            <h2 class="text-base font-semibold text-slate-900 lg:text-lg">{{ currentNavLabel }}</h2>
          </div>
